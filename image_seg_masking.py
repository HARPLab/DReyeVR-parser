...
def check_frame_quality(frame_num, recording_data_dict, sensor_config, inst_img_path):
    num_peds = 0
    min_num_peds = 2
    min_dist_bn_peds = 20
    peds_locs = []
...
def get_instseg_id_offset(recording_data_dict, images_dir, sensor_config, awareness_df):
    ...
    for frame_num in range(100, len(awareness_df)):
        ...
        if os.path.exists(inst_img_path):
            ...
            good, peds_dict = check_frame_quality(frame_num, recording_data_dict, sensor_config, inst_img_path)
            if good:
                f_num = frame_num
                break
